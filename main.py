...
async def get_user_rated_movies_endpoint(
    ...
    try:
        ...
        # Convert to list of dictionaries with consistent types and handle NaN values
        converted_movies = []
        for movie in rated_movies:
            converted_movie = {
                "id": int(movie.get('id', 0)) if pd.notna(movie.get('id')) else 0,
                "tmdb_id": int(movie.get('tmdb_id', 0)) if pd.notna(movie.get('tmdb_id')) else 0,
                "imdb_id": str(movie.get('imdb_id', '')) if pd.notna(movie.get('imdb_id')) else '',
                "title": str(movie.get('title', '')) if pd.notna(movie.get('title')) else '',
                "release_date": str(movie.get('release_date', '')) if pd.notna(movie.get('release_date')) else '',
                "genres": [str(g) for g in movie.get('genres', []) if pd.notna(g)],
                "director": str(movie.get('director', '')) if pd.notna(movie.get('director')) else '',
                "cast": [str(c) for c in movie.get('cast', []) if pd.notna(c)],
                "overview": str(movie.get('overview', '')) if pd.notna(movie.get('overview')) else '',
                "vote_average": float(movie.get('vote_average', 0.0)) if pd.notna(movie.get('vote_average')) else 0.0,
                "user_rating": float(movie.get('user_rating', 0.0)) if pd.notna(movie.get('user_rating')) else 0.0,
                "rating_timestamp": int(movie.get('rating_timestamp', 0)) if pd.notna(movie.get('rating_timestamp')) else 0
            }
